...
uint16_t X_DAC = 0x14; // Binary: 0001010 (<< 1)
uint16_t Y_DAC = 0x9A; // Binary: 1001101 (<< 1)
uint16_t Z_DAC = 0x9C; // Binary: 1001110 (<< 1)

uint16_t hcc_dac_packet(uint8_t volt_sp){
	...
	uint16_t packet = volt_sp << 4;

	return (packet); // Shift the 8 bit DAC Value left by 4 filling new empty spaces as zero: https://en.wikipedia.org/wiki/Bitwise_operations_in_C
}

void hcc_dac_set_output(hcc_i2c_t *i2c, uint16_t dac, uint8_t volt_sp){

	uint16_t data = hcc_dac_packet(volt_sp);
